...
let _colaProgreso = Promise.resolve();
function enColaProgreso(fn) {
  const siguiente = _colaProgreso.then(fn, fn);
  _colaProgreso = siguiente.catch(() => {});
  return siguiente;
}
...
export async function registrarRespuesta({ store, item, correcta, tema, ref }) {
  ...
  if (item && item.estadisticas_usuario) {
    item.estadisticas_usuario.veces_respondida = (item.estadisticas_usuario.veces_respondida || 0) + 1;
    if (correcta) item.estadisticas_usuario.veces_correcta = (item.estadisticas_usuario.veces_correcta || 0) + 1;
  ...
  // 3) progreso global — serializado para evitar race
  return enColaProgreso(async () => {
...
export async function leerProgreso() {
  // Esperar a que la cola se vacíe para devolver el estado más reciente.
  await _colaProgreso;
